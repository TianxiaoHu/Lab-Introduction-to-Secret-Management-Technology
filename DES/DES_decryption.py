...
def split(ciphertext):
	...
	if len(ciphertext) % 64 == 0:
		...
		ciphertext += '0' * (64 - (len(ciphertext) % 64))
	cipherbinarr = np.array(map(lambda x: int(x), ciphertext))
	return cipherbinarr.reshape((len(cipherbinarr) / 64, 64))
...
def DES(keys, smalltextbin):
	...
	for i in range(16):
		...
		text_B_xor = text_B_extend ^ keys[15 - i]
...
def main():
	...
	plaintext = bin2asc(ciphertext)

	print 'Key(hex):\t', sys.argv[1]
	print 'Ciphertext:\t', sys.argv[2]
